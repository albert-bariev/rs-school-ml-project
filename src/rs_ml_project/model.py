...
def kfold_cv(args: argparse.Namespace) -> None:
    ...
    scores = cross_validate(
        ...
        y,
        cv=KFold(
            n_splits=args.cross_validation_outer,
            shuffle=True,
...
def nested_cv(args: argparse.Namespace) -> None:
    ...
    params["1_cv_outer"] = args.cross_validation_outer
    params["1_cv_inner"] = (
        args.cross_validation_inner if args.cross_validation_inner else 3
    )
    params["2_scaler"] = args.scaler
    params["2_dim_reduced"] = args.dim_reduced
    params["2_feature_selector"] = args.feature_selector
    ...
    for train_ix, test_ix in cv_outer.split(X):
        ...
        pipeline = create_pipeline(
            model,
            params["2_scaler"],
            params["2_dim_reduced"],
            params["2_feature_selector"],
            params["2_kbest"],
        )
        search = GridSearchCV(
            pipeline,
            space[params["0_model"]],
            scoring="accuracy",
            cv=cv_inner,
            refit=True,
            n_jobs=-1,
